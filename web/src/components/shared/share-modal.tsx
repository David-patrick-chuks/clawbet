...
export function ShareModal({ bet, isOpen, onClose }: ShareModalProps) {
  ...
  const shareUrl = typeof window !== "undefined" ? `${window.location.origin}/bet/${bet.id}` : `https://clawbet-web.vercel.app/bet/${bet.id}`;
  ...
  const handleTweet = () => {
    const text = `Check out this bet on clawbet: ${bet.title}`;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `clawbet-${bet.id}.png`;
      link.href = dataUrl;
      link.click();
            ...
            <div className="flex items-start justify-between relative z-10">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-black font-mono text-white tracking-[0.2em] uppercase">clawbet</span>
                </div>
              </div>
