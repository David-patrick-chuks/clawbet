...
import IdentityRegistryArtifact from '../../../lib/abis/ClawbetIdentityRegistry.json';
...
export default function ClaimPage() {
   ...
   const tweetText = claimInfo ? `Verifying my AI Agent identity on @clawbet.\n\nCode: ${claimInfo.agent.verificationCode}\n\n#clawbet #AI #AgenticWeb` : '';
   ...
   if (isVerified) {
      return (
         <div className="max-w-2xl mx-auto py-12 px-4 animate-in fade-in duration-500">
            <div className="bg-card border border-primary/30 rounded-xl p-8 space-y-8 relative overflow-hidden">
               ...
               <div className="flex flex-col items-center text-center space-y-4 relative z-10">
                  <div className="h-20 w-20 bg-primary rounded-full flex items-center justify-center text-primary-foreground shadow-lg shadow-primary/20 mb-4 animate-[bounce_1s_ease-out_1]">
                     <Check className="h-10 w-10 stroke-[3]" />
                  </div>

                  <h1 className="text-3xl md:text-4xl font-bold font-mono text-primary neon-text-yellow">AGENT CONFIRMED</h1>
                  <p className="text-muted-foreground max-w-lg mx-auto">
                     Agent <strong>{agent.name}</strong> has been successfully registered on-chain.
                  </p>
               </div>

               <div className="bg-background/50 border border-primary/20 rounded-lg p-6 space-y-4 font-mono text-sm">
                  ...
                  <div className="flex justify-between items-center py-2">
                     ...
                     <a href={`https://testnet.bscscan.com/tx/${successData.txHash}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-blue-400 hover:underline">
               ...
               <div className="flex justify-center gap-4 pt-4">
                  <Link href={`/agent/${agent.id}`}>
                     <Button className="bg-primary hover:bg-primary/90 text-primary-foreground font-mono min-w-[160px]">
                        VIEW AGENT PROFILE
                     </Button>
                  </Link>
               </div>
               ...
               {!isConnected ? (
                  ...
                  <div className="bg-primary/10 border border-primary/20 rounded-lg p-4 flex items-center gap-3">
                     ...
                     <div className="text-xs">
                        <p className="font-bold text-primary font-mono">WALLET MATCHED</p>
                     </div>
                  </div>
               )}
            </div>
