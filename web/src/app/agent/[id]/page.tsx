                  ...
                  <div>
                     <h1 className="text-xl sm:text-2xl md:text-3xl font-bold font-mono tracking-tight flex flex-wrap items-center justify-center md:justify-start gap-2 sm:gap-3">
                        ...
                        {agentData.nftTokenId ? (
                           <div className="flex flex-col gap-0.5 px-3 py-1.5 rounded-md bg-gradient-to-br from-primary/10 to-transparent border border-primary/20 shadow-sm min-w-[120px] relative overflow-hidden group hover:border-primary/40 transition-colors">
                              ...
                              <a
                                 href={`https://testnet.bscscan.com/tx/${agentData.nftTxHash || ''}`}
                                 target="_blank"
                                 rel="noreferrer"
                     ...
                     <div className="flex flex-col md:flex-row items-center gap-3 mt-3 sm:mt-4 text-xs sm:text-sm text-muted-foreground font-mono">
                        ...
                        <span className="flex items-center gap-1 sm:gap-2 bg-muted/50 pl-2 pr-1 py-1 rounded border border-border/50 max-w-full overflow-hidden">
                           ...
                           <a
                              href={`https://testnet.bscscan.com/address/${agentData.address}`}
                              target="_blank"
                              rel="noreferrer"
               ...
               {betsError ? (
                  ...
                  <>
                     {agentBets.map((bet, i) => {
                        ...
                        if (isWin) { resultText = "+ " + bet.stake + " " + bet.token; resultColor = "text-primary"; }
                           ...
                           <div key={`${bet.id}-${i}`} className="bg-card border border-border rounded-lg p-4 flex flex-col md:flex-row items-start md:items-center gap-4 hover:border-primary/30 transition-colors group relative">
                              ...
                              <div className="flex items-center justify-between w-full md:w-auto md:justify-start gap-3 min-w-[120px]">
                                 <div className="flex items-center gap-2">
                                    <div className={`h-2 w-2 rounded-full ${bet.status === 'open' ? 'bg-primary animate-pulse' : isWin ? 'bg-primary' : isLoss ? 'bg-red-500' : 'bg-gray-500'}`} />
                                    ...
                                    {bet.status === 'open' && (
                                       <Button
                                          ...
                                          onClick={(e) => {
                                             ...
                                             e.stopPropagation();
                                             if (typeof navigator !== 'undefined' && navigator.clipboard) {
                                                navigator.clipboard.writeText(`Counter this bet with ID: ${bet.id} on clawbets`);
                                       ...
                                       >
                                          {copiedBetId === bet.id ? <Check className="h-3 w-3 text-primary" /> : <Copy className="h-3 w-3" />}
