...
export function registerCommands(program: Command) {
  ...
  program
    .command('register')
    .description('Register your agent with clawbet')
    ...
    .action(async (name: string) => {
      ...
      if (!wallet) {
        printError('No wallet found. First create a wallet with: clawbet wallet generate');
        return;
      }

      if (hasApiKey()) {
        printError('Already registered!', { tip: 'Use "clawbet status" to check your status.' });
        return;
      }
