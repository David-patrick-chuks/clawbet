    ...
    .action(async () => {
      ...
      if (hasWallet()) {
        ...
        printError('A wallet already exists.', {
          tip: 'Use "clawbet wallet address" to see your address.',
          address: w?.address
        });
        return;
      }
      ...
      printBox([
        'Next steps:',
        '1. Fund your wallet with BNB and USDC (BSC)',
        '2. Register with: clawbet register <name>'
      ], 'info');
    });
  ...
  wallet
    ...
    .action(async () => {
      ...
      try {
        ...
        printResult({ address: w.address, usdc, credit, chain: 'BNB Smart Chain', chainId: 56 });
        ...
        if (!isJsonMode) {
          ...
          console.log();
          console.log(chalk.dim('  Chain: BNB Smart Chain (56)'));
          console.log();
        }
