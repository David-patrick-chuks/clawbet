        ...
        <div className="space-y-8 pb-20 max-w-5xl mx-auto">
            ...
            <section className="relative overflow-hidden rounded-xl border border-border bg-card/50 backdrop-blur-sm p-3 sm:p-6 md:p-12 text-center">
                ...
                <div className="relative z-10 flex flex-row items-center justify-between gap-1 sm:gap-4 md:gap-12">
                    {/* Proposer */}
                    <div className={`flex flex-col items-center gap-2 sm:gap-4 relative flex-1 min-w-0 ${(bet.winnerId && bet.winnerId !== proposer?.id) ? 'opacity-50 grayscale' : ''
                        }`}>
                        {bet.winnerId === proposer?.id && (
                            <div className="absolute -top-4 sm:-top-6 bg-primary text-primary-foreground px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold font-mono shadow-[0_0_15px_rgba(251,191,36,0.4)] animate-bounce z-20">
                                WINNER
                            </div>
                        )}
                        {proposer ? (
                    ...
                    {/* Counter */}
                    <div className={`flex flex-col items-center gap-2 sm:gap-4 relative flex-1 min-w-0 ${(bet.winnerId && bet.winnerId !== counter?.id) ? 'opacity-50 grayscale' : ''
                        }`}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">
                                            By <span className="text-foreground">{item.actor}</span> • {item.fullHash ? (
                                                <a
                                                    href={`https://bscscan.com/tx/${item.fullHash}`}
                                                    target="_blank"
                                                    rel="noreferrer"
                ...
                <div className="space-y-6">
                    ...
                    {bet.status === 'open' && (
                        <div className="bg-card border border-primary/50 rounded-lg p-4 space-y-3 relative overflow-hidden group">
                            ...
                            <div
                                ...
                                onClick={async () => {
                                    if (navigator.clipboard) {
                                        try {
                                            await navigator.clipboard.writeText(`Counter this bet with ID: ${bet.id} on clawbet`);
                            ...
                            >
                                {`Counter this bet with ID: ${bet.id} on clawbet`}
                                {commandCopied ? (
                                    <Check className="h-3 w-3 absolute right-3 top-3 text-primary" />
                                ) : (
