...
import { evaluateDispute } from '../services/judge';
...
// ─────────────────────────────────────────────────────────────────────────────
// POST /api/admin/disputes/:id/ai-resolve - Get AI opinion on dispute
// ─────────────────────────────────────────────────────────────────────────────

const aiResolveDisputeRoute = createRoute({
  method: 'post',
  path: '/disputes/:id/ai-resolve',
  tags: ['Admin', 'Disputes'],
  summary: 'Get AI Judge verdict',
  security: [{ cookieAuth: [] }],
  request: {
    params: z.object({ id: z.string() }),
  },
    ...
    200: {
      ...
      content: {
        'application/json': {
          schema: SuccessResponseSchema(z.object({
            verdict: z.object({
              winner: z.enum(['proposer', 'counter', 'uncertain']),
              confidence: z.number(),
              reasoning: z.string()
            }),
            suggestedWinnerId: z.string().nullable()
          })),
        },
      },
    },
...
app.openapi(aiResolveDisputeRoute, async (c) => {
  const { id: disputeId } = c.req.valid('param');
  const db = getDb();

  // Get dispute with bet and relationships
  const dispute = await db.query.disputes.findFirst({
    where: eq(disputes.id, disputeId),
    with: {
      bet: {
  ...
  const context = {
    ...
    betTerms: bet.terms,
    claimerEvidence: bet.winClaimEvidence || 'No evidence provided by claimer.',
    disputerReason: dispute.reason,
    disputerEvidence: dispute.evidence || undefined,
  };
  ...
  if (verdict.winner === 'proposer') suggestedWinnerId = bet.proposerId;
  if (verdict.winner === 'counter') suggestedWinnerId = bet.counterId || null;
